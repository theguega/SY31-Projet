...
class MappingNode:
    ...
    def callback_final(self, msg):
        ...
        points_msg = create_cloud(
            msg.header,
            PC2FIELDS,
            [[points[i, 0], points[i, 1], 0, 0] for i in range(points.shape[0])],
        )

        # Add points to the map
        self.map = np.vstack((self.map, points)) if self.map.size else points

        map_msg = create_cloud(
            msg.header,
            PC2FIELDS,
            [[self.map[i, 0], self.map[i, 1], 0, 0] for i in range(self.map.shape[0])],
        )
